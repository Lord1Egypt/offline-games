class Board extends Phaser.GameObjects.Container{
        ...
        for(let x = 0; x < solvedState.length; x++){
            for(let y = 0; y < solvedState[x].length; y++){
                ...
                if(cellData != null){
                    ...
                    boardTile.originalScale = boardTile.scale;
    ...
    freeBoardTiles(){
        ...
        for(let i = 0; i < keys.length; i++){
            ...
            if(boardTile.txt){
                boardTile.txt.setVisible(false);
                boardTile.txt.setActive(false);
            }
    ...
    removePreviewTile(boardTile){
        if(boardTile.cellData.previewTile != null){
            boardTile.cellData.previewTile.parentContainer.remove(boardTile.cellData.previewTile);
    ...
    revealLetter(){
        if(this.txt == null){
            this.txt = new Phaser.GameObjects.Text(this.scene,
                ...
                0,
                " ",
                {fontFamily: FONT, fontSize: 50, color: '#999', align: 'center', fontStyle: "bold" }
            ).setOrigin(0.5, 0.5)
